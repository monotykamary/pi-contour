...
var s=e=>e.replace(/[\u0000-\u001f\u007f-\u009f]/g,o=>`\\u${o.charCodeAt(0).toString(16).padStart(4,"0")}`),d=e=>`${e>=0?"+":""}${e}`,u=e=>{let o=e.evidence.map(i=>`  \u25AA ${s(i.file)}:${i.line}${i.relation?` [${s(i.relation)}]`:""}${i.witness?` \u2014 ${s(i.witness).slice(0,180)}`:""}`).join(`
...
${o}
  ...
  \u2197 Exposure (modeled): ${t}`:"")+`
  ? ${s(e.question)}`};function h(e,o=2500){if(!Number.isInteger(o)||o<256||o>16e3)throw new Error("maxTokens must be 256..16000");let t=e.coverage.before.length+e.coverage.after.length,i=s(e.root),g=i.length>240?i.slice(0,110)+"\u2026"+i.slice(-120):i,c=`Contour \xB7 ${e.target} \xB7 ${e.baseline.slice(0,12)} \u2192 ${e.snapshot.slice(0,12)}
Root: ${g}
...
`,l=e.findings.map(u),r=n=>`
${e.totalFindings-n} findings not displayed (CLI --json for details).
`+(t?`\u26A0 Coverage incomplete; absence of findings is not approval.
`:"")+"No quality score or correctness guarantee. Graph resolution is partial. Heat means exposure, not defect probability.",a=c+r(0),$=0;for(let n=1;n<=l.length;n++){let f=c+`
`+l.slice(0,n).join(`

`)+r(n);if(Math.ceil(f.length/4)>o)break;a=f,$=n}let b=[...e.coverage.before.slice(0,3).map(n=>`\u26A0 Gap (baseline): ${s(n.file)} \u2014 ${s(n.reason).slice(0,200)}`),...e.coverage.after.slice(0,3).map(n=>`\u26A0 Gap (target): ${s(n.file)} \u2014 ${s(n.reason).slice(0,200)}`),...e.regions.slice(0,5).map(n=>`\u0394 Region ${s(n.region)}: decisions ${n.before.decisions} \u2192 ${n.after.decisions}; functions ${n.before.functions} \u2192 ${n.after.functions}`)];for(let n of b){if(Math.ceil((a.length+n.length+1)/4)>o)break;a+=`
${n}`}return{text:a,tokens:Math.ceil(a.length/4),displayed:$}}export{h as a};
