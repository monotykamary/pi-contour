...
`);if(!e.startsWith(i)||a.slice(0,s)!==`# sha256:${f(i+a.slice(s+1))}`)throw new Error("Refusing to remove a modified or unmanaged Contour hook");return await A(r),`Removed ${r}`}async function W(){return"0.3.0"}var K=`contour review [--staged|--working-tree] [--json] [--root DIR]
