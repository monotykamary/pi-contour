...
const renderFinding = (finding: Finding): string => {
  const evidence = finding.evidence.map(e => `  ${clean(e.file)}:${e.line}${e.witness ? ` — ${clean(e.witness).slice(0, 180)}` : ""}`).join("\n");
  const exposure = finding.exposure?.map(e => `t=${e.time}: ${(100 * e.outsideRegion).toFixed(1)}% outside source region`).join("; ");
  return `[${finding.category}${finding.blocking ? "; explicit policy" : "; advisory"}] ${clean(finding.title)}\n${evidence}\n  ${finding.before} → ${finding.after} ${finding.unit}\n  ${finding.question}${exposure ? `\n  Exposure: ${exposure}` : ""}`;
};
...
export function renderReport(report: ReviewReport, maxTokens = 2500): { text: string; tokens: number; displayed: number } {
  ...
  const header = `Contour · ${report.target} · ${report.baseline.slice(0, 12)} → ${report.snapshot.slice(0, 12)}\n`
    ...
    + `SLOC ${signed(report.after.sloc - report.before.sloc)}; decisions ${signed(report.after.decisions - report.before.decisions)}; `
    ...
    + `Verbose lines ${report.before.verboseLines} → ${report.after.verboseLines}; complex mass ${report.before.complexMass.toFixed(1)} → ${report.after.complexMass.toFixed(1)}\n`;
  const details = report.findings.map(renderFinding);
  const footer = (n: number): string => `\n${report.totalFindings - n} findings not displayed (CLI --json for details). ${gaps ? "Coverage incomplete; absence of findings is not approval." : "No quality score or correctness guarantee."} Graph resolution is partial. Heat means exposure, not defect probability.`;
  ...
  // Coverage witnesses have priority over optional regional summaries when budget is tight.
  const extras = [...report.coverage.after.slice(0, 3).map(gap => `Gap: ${clean(gap.file)} — ${clean(gap.reason).slice(0, 200)}`),
    ...report.regions.slice(0, 5).map(r => `Region ${clean(r.region)}: decisions ${r.before.decisions} → ${r.after.decisions}; functions ${r.before.functions} → ${r.after.functions}`)];
