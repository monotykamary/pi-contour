...
- `bun run cover`: regenerate the SVG; verify XML, static fallback, reveal, and reduced-motion rendering after visual edits. Refresh `media/preview.png` from the reduced-motion composition for Pi's static image gallery (SVG remains the animated README artwork). Keep GitHub display-math blocks balanced and valid.
